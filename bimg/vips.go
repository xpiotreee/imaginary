...
func vipsRead(buf []byte) (*C.VipsImage, ImageType, error) {
	return vipsReadWithDPI(buf, 0)
}

func vipsReadWithDPI(buf []byte, dpi int) (*C.VipsImage, ImageType, error) {
	...
	err := C.vips_init_image(imageBuf, length, C.int(imageType), &image, C.double(float64(dpi)))
	if err != 0 {
